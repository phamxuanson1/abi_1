...
public class Enemy : Character
{
    ...
    {
        // Kiểm tra nếu state mới trùng loại với state hiện tại thì không làm gì cả
    ...
    {
        // Kiểm tra nếu target không null thì mới tính khoảng cách
        if (target != null)
        {
            return Vector2.Distance(target.transform.position, transform.position) <= attackRange;
        }

        // Nếu không có target thì chắc chắn không ở trong tầm đánh
        return false;
    }
